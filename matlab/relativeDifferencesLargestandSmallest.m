...
for index=1:length(subjectIDandStatusandTask(:,1))
    tasksOfSubject= cell2mat(subjectIDandStatusandTask(index,3)); %all images of subject ID 'index'
    indexAnnotations = find(ismember(filterDataTable(:,1), tasksOfSubject)); %alle annotaties van subject ID
    ...
    subjectID=cell2mat(subjectIDandStatusandTask(index,1));
    disp( [num2str(length(indexLargestAirway)), ' annotations available of the largest airway and ', num2str(length(indexSmallestAirway)), ' annotations of the smallest airway of subject ID ' num2str(subjectID)])
