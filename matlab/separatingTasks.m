...
numTasks=2380; %the total number of images in the original dataset
...
%GtTable. 
%Unit in the second column contains a column with the annotated inner areas
%of that task
...
for task=1:numTasks;
    ...
    for j=1:length(indexesAnnotations)
        ...
        InnerAreasTask=[InnerAreasTask; filterAnnotTable(indexAnnotation,1)]; %inner area of task/ image slice
        OuterAreasTask=[OuterAreasTask; filterAnnotTable(indexAnnotation,2)]; %outer area of task/ image slice
        WAPTask=[WAPTask; filterAnnotTable(indexAnnotation,3)];               %Wall area percentage of task/ image slice
